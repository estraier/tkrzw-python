...
Whereas Tkrzw is C++ library, this package provides its Python interface.  All above data structures are available via one adapter class "L{DBM}".  Read the U{homepage <http://dbmx.net/tkrzw/>} for details.
...
An instance of the class "L{DBM}" is used in order to handle a database.  You can store, delete, and retrieve records with the instance.  The result status of each operation is represented by an object of the class "L{Status}".  Iterator to access access each record is implemented by the class "L{Iterator}".
...
class DBM:
  ...
  def Open(self, path, writable, **params):
    ...
    @note: The optional parameter "dbm" supercedes the decision of the database type by the extension.  The value is the type name: "HashDBM", "TreeDBM", "SkipDBM", "TinyDBM", "BabyDBM", "CacheDBM", "StdHashDBM", "StdTreeDBM".
    @note: For HashDBM, these optional parameters are supported.
    ...
    @note: For TreeDBM, all optional parameters for HashDBM are available.  In addition, these optional parameters are supported.
    ...
    @note: For SkipDBM, these optional parameters are supported.
      - offset_width (int): The width to represent the offset of records.
    ...
    @note: For TinyDBM, these optional parameters are supported.
      - num_buckets (int): The number of buckets for hashing.
    @note: For BabyDBM, these optional parameters are supported.
      - key_comparator (string): The comparator of record keys. The same ones as TreeDBM.
    @note: For CacheDBM, these optional parameters are supported.
      - cap_rec_num (int): The maximum number of records.
      - cap_mem_size (int): The total memory size to use.
    @note: If the optional parameter "num_shards" is set, the database is sharded into multiple shard files.  Each file has a suffix like "-00003-of-00015".  If the value is 0, the number of shards is set by patterns of the existing files, or 1 if they doesn't exist.
    """
    pass
  ...
  def CompareExchange(self, key, expected, desired):
    ...
    @return: The result status.
    @note: If the record doesn't exist, NOT_FOUND_ERROR is returned.  If the existing value is different from the expected value, DUPLICATION_ERROR is returned.  Otherwise, the desired value is set.
    """
    pass
  ...
  def Rebuild(self, **params):
    ...
    @param params: Optional parameters.
    @return: The result status.
    @note: The optional parameters are the same as the Open method.  Omitted tuning parameters are kept the same or implicitly optimized.
    """
    pass
  ...
  def Synchronize(self, hard, **params):
    ...
    @param params: Optional parameters.
    @note: Only SkipDBM uses the optional parameters.  The "reducer" parameter specifies the reducer to apply to records of the same key.  "ReduceToFirst", "ReduceToSecond", "ReduceToLast", etc are supported.
    """
    pass
...
class Iterator:
  ...
  def Last(self):
    ...
    @return: The result status.
    @note: Even if there's no record, the operation doesn't fail.  This method is suppoerted only by ordered databases.
    """
    pass
  ...
  def Jump(self, key):
    ...
    @return: The result status.
    @note: Ordered databases can support "lower bound" jump; If there's no record with the same key, the iterator refers to the first record whose key is greater than the given key.  The operation fails with unordered databases if there's no record with the same key.
    """
    pass
  ...
  def JumpLower(self, key, inclusive=False):
    ...
    @return: The result status.
    @note: Even if there's no matching record, the operation doesn't fail.  This method is suppoerted only by ordered databases.
    """
    pass
  ...
  def Next(self):
    ...
    @return: The result status.
    @note: If the current record is missing, the operation fails.  Even if there's no next record, the operation doesn't fail.
    """
    pass
  ...
  def Previous(self):
    ...
    @return: The result status.
    @note: If the current record is missing, the operation fails.  Even if there's no previous record, the operation doesn't fail.  This method is suppoerted only by ordered databases.
    """
    pass
